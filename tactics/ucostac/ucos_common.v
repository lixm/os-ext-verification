...
Proof.
  ...
  destruct e.
  simp join.
  match goal with H: ECBList_P _ _ _ _ _ _ |- _ => apply IHl2 in H end.
  lia.
Qed.
...
Lemma R_Prio_NoChange_Prio_hold:
  forall tcb tid st prio st',
    R_Prio_No_Dup tcb ->
    TcbMod.get tcb tid = Some (prio, st) ->
    R_Prio_No_Dup (TcbMod.set tcb tid (prio, st')). 
Proof.
  introv Hrp Htcb.
  unfolds.
  unfolds in Hrp.
  ...
  lets Hba :  tcbmod_neq_set_get tid0 tid  (prio', st'0)  tcb H.
  ...
  tryfalse.
  lets Hba :  tcbmod_neq_set_get tid' tid (prio, st') tcb H2.
  unfold get in *. simpl in *.
  rewrite H1 in Hba.
  apply eq_sym in Hba.
  ...
  lets Hbba :  tcbmod_neq_set_get tid0 tid (prio, st') tcb H3.
  rewrite H0 in Hbba.
...
Lemma R_PrioTbl_P_impl_prio_neq_cur :
  forall ptbl tcbls curtid curprio st vhold,
    0 <= Int.unsigned curprio < 64 ->
    R_PrioTbl_P ptbl tcbls vhold ->
    TcbMod.get tcbls curtid = Some (curprio, st) ->
    prio_neq_cur tcbls curtid curprio.
Proof.
  introv Hran Hrp Htc.
  unfolds in Hrp.
...
Lemma join_prop1:
  forall v'44 v'43 v'7 v'58 prio st v'59 v'49 v'47,
...
Lemma join_prop2_my:
  forall m1 m2 m12 b1 prio st m3 ma3 m4,
    ...
    TcbMod.join m1 
                (TcbMod.set m2 (b1, Int.zero) (prio, rdy))
                (TcbMod.set m12 (b1, Int.zero) (prio, rdy)). 
Proof.
  unfold TcbJoin.
  intros.
  eapply TcbMod.my_join_sig_abc.
...
Lemma join_prop2: forall v'44 v'43 v'7 v'57 prio st v'58 v'48 v'46,
                    TcbMod.join v'44 v'43 v'7 ->
                    TcbJoin (v'57, Int.zero) (prio, st) v'58 v'48 ->
                    TcbMod.join v'46 v'48 v'43->
                    TcbMod.join v'44
                                (TcbMod.set v'43 (v'57, Int.zero) (prio, rdy))
                                (TcbMod.set v'7 (v'57, Int.zero) (prio, rdy)).
Proof.
  apply join_prop2_my.
Qed.
...
Lemma astruct_neq_ptr :
  forall s x id1 t1 d1 id2 t2 d2 v1 y  v2 P a b,
    ~array_struct t1 ->
    ~ array_struct t2 ->
...
Lemma ecbf_sllseg_isptr :
  forall s P v1 v2 t f,
    s |= ecbf_sllseg v1 Vnull v2 t  f ** P -> isptr v1.
Proof.
  intros.
  gen v1 t f P.
  inductions v2.
...
Lemma evsllseg_add_head:
  forall head tailnext vl ecbl s P tl l x msgq,
    V_OSEventListPtr l = Some head ->
    s|= AEventNode tl l x msgq ** evsllseg head tailnext vl ecbl ** P ->
    s|= evsllseg tl tailnext ((l, x) :: vl) (msgq :: ecbl) ** P.
  intros.
  unfold evsllseg in *.
  sep auto.
Qed.
...
Lemma aeventnode_evsllseg_get_neq :
  forall ectrl edatal head head' n v v0 e x tail s P,
...
Proof.
  ...
  destruct v7; simpl in H0; tryfalse.
...
Lemma aeventnode_evsllseg_get_neq' :
  forall ectrl edatal head head' n v v0 e x tail s,
...
Lemma evsllseg_eid_neq:
  forall ectrl edatal head tail s P,
    s |= evsllseg head tail ectrl edatal ** P ->
    vl_elem_neq (get_eid_list head ectrl).  
Proof.
   inductions ectrl; intros.
  simpl in H; simp join.
  unfolds; intros.
...
Lemma ecblist_p_decompose:
  forall l1 ll1 l2 ll2 head ecbls tcbls,
    ...
    ECBList_P head Vnull (l1++ l2) (ll1 ++ ll2) ecbls tcbls ->
...
Lemma tcbdllseg_split:
  forall x1 s P v'23 v'32 x2 x3 xx,
    ...
    s |=
      EX x v'15, tcbdllseg (Vptr v'23) xx x (Vptr v'15) x1 **
                   tcbdllseg (Vptr v'15) x v'32 Vnull (x2 :: x3) **
                   [|x1<>nil /\ nth_val 0 (last x1 nil) = Some (Vptr v'15) \/ x1 = nil /\ Vptr v'15 = Vptr v'23 |] ** P.
Proof.
  induction x1.
  intros.
  simpl List.app in H.
