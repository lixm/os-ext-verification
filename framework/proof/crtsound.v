...
Proof.
  ...
  left.
  sep split; auto.
  assert ((gettopis si + 1) = S (gettopis si) ) by lia.
  rewrite H0 .
  ...
  introv Hmdisj Hinvv Hdisjj Hnabt'. 
...
Proof.
  ...
  replace (INUM-0) with INUM by lia.
  unfold inv_ieoff in Hoff.
  sep split in Hoff.
  sep destruct Hoff.
  sep split in Hoff.
  ...
  simp join.
  rewrite H20 in *.
  replace (S x18) with (S (( gettopis si) +(S x18 - (gettopis si + 1)))) by lia.
  apply starinv_isr_elim1 with (i := gettopis si)(j := (S x18 - (gettopis si + 1))).
  replace (S (gettopis si)) with ((gettopis si) +1) by lia.
  ...
  splits; eauto.
  replace (gettopis si) with (gettopis si-0) by lia.
  eapply inv_isr_prop ; eauto.
  eapply inv_isr_prop ; eauto.
  sep split in H18.
  rewrite H17 in *.
  ...
  subst.
  replace (INUM) with (INUM-0) by lia.
  eapply inv_isr_prop ; eauto.
  eapply  Linv_igore; eauto.
  unfolds.
  intros.
