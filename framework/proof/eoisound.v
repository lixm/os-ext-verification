...
Proof.
  ...
  lets Hle:    Zle_imply_natle H1 H6.
  ...
  replace (INUM -0)%nat  with (INUM) in * by lia.
  clear H.
  exists aop OO.
  ...
  destruct H9 as (Oss & Hjj1 & Hjj2).

  exists (x4, x5, x0,  isrupd x8 (Z.to_nat (Int.intval i)) false,
          (true, m :: is, cs) ).
  exists Mss x3 Oss.
  unfold INV in *.
  ...
  unfolds.
  intros.
  assert (exists k, join x1 k  Mss/\ join x x9 k).
  clear -H8 Hj2.
  join auto.
  assert (exists k, join x11 k  Oss/\ join x2 x13 k).
  clear -H11 Hjj2.
  join auto.
  simp join.
  exists x1 x14  Mss x11 x10 Oss.
  splits;eauto.
  simpl substmo.
  ...
  rewrite Heqk in *.
  assert (S (S k) = ((S m%nat) + (S (k-m)))%nat) by lia.
  rewrite H16 in *.
  ...
  destruct H26.
  mytac.
  assert (exists k, join x15 k x14/\ join x x16 k).
  clear -H15 H18.
  join auto.
  assert (exists k, join x18 k x10/\ join x2 x19 k).
  clear -H14 H20.
  join auto.
  simp join.
  exists x15 x17 x14 x18 x8 x10.
  splits; eauto.
  simpl substmo.
  replace (S m) with (S( m + 0))%nat  in * by lia.
  eapply  starinv_isr_prop5; eauto.
  eapply inv_isr_prop; eauto.
  simpl substmo.
  exists x x16 x17 x2 x19 x8.
  splits; eauto.
  simpl .
  ...
  introv Hjj1 Hjj2 Hinv Hjoin' Hnabt. 
  introv Hstep.
  apply Hstep.
  destruct o2 as  [[[[]]]].
  do 2 eexists.
...
Proof.
  ...
  exists aop OO (x0, x1, x8, isrupd x4 (Z.to_nat (Int.unsigned i))false,
  ...
  unfold Int.unsigned in *.
  remember (Z.to_nat (Int.intval i)) as m.
  replace (m +1)%nat with (S m) in * by lia.
  eapply  starinv_isr_prop4; eauto.
  eapply inv_isr_prop; eauto.
  sep split; auto.
  right.
