...
Lemma cstep_cont_locality :
  forall po pi c c' ke ke' ks ks' ks'' m m',
...
Lemma loststep_cont_locality:
  forall c ke ks o o' c' ke' ks' po pi ks'',
    loststep (pumerge po pi) (c,(ke,ks)) o (c',(ke',ks')) o' ->
    loststep (pumerge po pi) (c,(ke,ks ## ks'')) o (c',(ke',ks' ## ks'')) o'. 
Proof.
  intros.
  inv H; 
...
Proof.
  ...
  eapply hp_stepO.
  intros.
  eapply hp_stepS with (T':=TasksMod.set Th t c') (cst':=cst') (O':=O'); eauto. 
  constructors; eauto.
  assert (TasksMod.set Th t c'' =TasksMod.set (TasksMod.set Th t c') t c'').
  apply tasks_set_set.
  rewrite H4.
  apply IHhtstepstar;auto.
  apply htstep_tidsame in H0.
  unfold tidsame in H0.
  rewrite <- H0 ;auto.
  apply tasks_get_set.
Qed.
...
Proof.
  ...
  apply IHhpstepstar in H4; eauto.
  eapply hp_stepS; eauto.
Qed.
...
Proof.
  ...
  constructors.
  intros.
  lets Hx:htstep_O_local H0 H2.
  mytac.
  apply IHhtstepstar in H4.
  mytac.
  eexists;split;eauto.
  eapply ht_starS;eauto.
Qed.
