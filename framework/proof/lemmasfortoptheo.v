...
Lemma kscall_concat: 
  forall ks ks' ks'' f s E c ke lenv, 
    ks ## kint c ke lenv ks' = kcall f s E ks'' ->
    exists ks_, ks = kcall f s E ks_. 
Proof.
  induction ks; 
    introv Heq; try (simpl in Heq; false; fail).
  simpl in Heq.
  inverts Heq.
  eexists; eauto.
Qed.
...
Proof.
  ...
  {
    intros.
    rename H10 into Hneapi.
    rename H11 into Htsknabt. 
    ...
    - (* c' is exint *) 
      ...
      eapply iret_spec with
        (m:=mi) (O:=O') (Os:=Os') (Ms:=Ms) (is':= si) (i:=i)
        (isrreg:= isrreg) (ab':=spec_done None) (MM:=merge mi Ms) (OO:=OO') in Hinv';eauto.
      ...
      eapply htstepstar_compose_tail with
        (c':=(curs (hapi_code (spec_done None)), (kenil, kevent ch keh ksh))) (O':=(merge Ol OO')). 
      ...
      { (* ~sfexec_enapi ... *) 
        introv Hf.
        unfolds in Hf. destruct A. destruct p0; simpljoin; tryfalse. 
      }
  ...
  (*--------------------------------------------*)
  {
    ...
    rename H10 into Hneapi.
    unfolds in Hneapi.
    destruct Hneapi as [Hf | Hf].
    - simpljoin. simpl in Hnocallc. simpljoin; false.
    - destruct Hf as (vl & dl & f & s & E & ks'0 & Hc' & Heq' & Hnone).
      simpl in Hnocallc.
      destruct Hnocallc as (Hncc & Hnck). 
      lets Hff: kscall_concat Heq'; eauto.
      simpljoin. 
      simpl in Hnck.
      simpljoin; false. 
  }
  ...
  { 
    ...
    destruct H7 as
      (gamma'&sleft&OO'&oll&Mc&O'&Os'&Oll&Oc&
         Hgammaeq&Hhm&Hlj&Hhj&Hhj'&Hinv&Hswinv&Hlinv&Hmsim).
    rename ge into GG.
    subst gamma'.
    destruct oll as [[[[]]]].
    unfolds in Hlj.
    ...
    splits;auto.
    intros.
    eapply osapi_lift' with
      (cst:=cst) (pc:=pc) (A:=A) (t:=t) (ke:=kenil) (ks:= kevent ch keh ksh) in Hhm;eauto. 
    ...
    intros.
    eapply CIH with
      (isrreg:=isrreg) (si:=si) (Ml:=Ml) (ab:=sleft)
      (oi:=(e, e0, merge Mll Mc', i0, l)) (Ol:=Ol) (Oi:=merge Oll Oc');
      eauto.
  ...
  { 
    ...
    eapply CIH with
      (isrreg:=isrreg) (si:=si) (Ml:=Ml) (ab:=sleft)
      (oi:=(e, e0, merge mi Mdel, i0, l)) (Ol:=Ol) (Oi:=merge O' Odel); eauto. 
    apply join_comm in H15.
    eapply disj_join_join_merge;eauto.
    rewrite disjoint_merge_sym in H15;auto.
    apply disj_sym;auto.
...
Proof.
  ...
  rewrite H22.
  eapply IntSeq'' with
    (absi:=x) (i:=i) (Ol:=Ol) (ge:=ge) (Ml:=Ml)
    (Oi:=merge Oi Olinv) (oi:=(ge, empenv, merge Mi Mlinv, isrupd ir i true, (false, i :: is, nil))) (isrreg:=ir) (lg:=lg) (si:=is);
    eauto.
  clear -Hnoos H30.
  unfolds in Hnoos.
  ...
  assert ((((ge, empenv, (merge Mi Mlinv),  isrupd ir i true, (false, i::is, nil)) ,
             (merge Oi Olinv), (x )) |= ipreasrt i ir is (x ) I ge lasrt t lg) /\
            satp (ge, empenv, (merge Mi Mlinv),  isrupd ir i true, (false, i::is, nil))  (merge Oi Olinv) (CurLINV lasrt t)).
  {
...
Lemma ltstep_n_dym_com_int_cd:  
  forall pc po pi ip t c' ke' ks' c'' ke'' ks'' cst o cst' o', 
    ...
    GoodClient pc po pi ->
    ltstep (pc,(po,pi,ip)) t (c', (ke', ks' )) cst o (c'', (ke'', ks'' )) cst' o' ->
...
Lemma ltstepev_n_dym_com_int_cd:  
  forall pc po pi ip t c' ke' ks' c'' ke'' ks'' cst o cst' o' ev, 
    ...
    (forall f t d1 d2 s, pi f = Some (t,d1,d2,s) -> good_decllist (revlcons d1 d2) = true/\GoodStmt' s) ->
    GoodClient pc po pi ->
    ltstepev (pc,(po,pi,ip)) t (c', (ke', ks' )) cst o (c'', (ke'', ks'' )) cst' o' ev -> 
    n_dym_com_int_cd (c', (ke', ks' )) ->
    n_dym_com_int_cd (c'', (ke'', ks'')).  
Proof.
  intros.
...
Lemma tlmatch_dec':
  forall tl dl (vl:vallist),
    ~(tlmatch tl dl/\ dl_vl_match dl (rev vl)=true) \/ (tlmatch tl dl /\ dl_vl_match dl (rev vl)=true).
Proof.
  intros.
...
Lemma funbody'_inos:
  forall s po pi f ft d1 d2 s',
    GoodStmt' s->
    po f = Some (ft,d1,d2,s') ->
    InOS (curs s,(kenil, kcall f s' empenv kstop)) (pumerge po pi). 
Proof.
  intros.
  unfold InOS.
...
Lemma XXXX:
  forall ksx f s kstop ks,
    ksx ## (kcall f s empenv kstop ## ks) =  ksx ## kcall f s empenv kstop ## ks. 
Proof.
  intro ksx.
...
Lemma cstep_sfe_sfe_false:
  forall pu x x0 x1 ke ks ke' ks' ge le M le' M' f vl tl, 
    cstep pu
      (curs (sfexec x x0 x1), (ke, ks)) (ge, le, M)
      (curs (sfexec f vl tl), (ke', ks')) (ge, le', M') -> False. 
Proof.
  introv Hf.
  inverts Hf.
  - inverts H7. 
...
Lemma eqdomOS_api_nnone_impl: 
  forall po0 pi0 ip0 o1 o3 o0 f, 
    eqdomOS (po0, pi0, ip0) (o1, o3, o0) ->
    o1 f <> None ->
    po0 f <> None.
Proof.
  introv Heq Hnnon.
  unfolds in Heq.
  destruct Heq as (Ha & _).
  specialize (Ha f).
  destruct Ha as (Hlr & Hrl).
  destruct (o1 f) eqn: E.
  -
    specializes Hrl; eauto.
    simpljoin.
    introv Hf.
    false.
  -
...
Lemma inos_enapi_false: 
  forall c ke ks pc po pi ip, 
    (~ InOS (c, (ke, ks)) (pumerge po pi)) ->
    enapi (c, (ke, ks)) (pc, (po, pi, ip)) ->
    False.
Proof.
  introv Hnio Hea.
  apply Hnio.
  unfolds in Hea.
  unfolds.
  destruct Hea; simpljoin.
  - 
    do 3 eexists.
    split; eauto.
    left.
    destruct (po x) eqn: E; tryfalse. 
    assert (exists y, pumerge po pi x = Some y).
    {
      unfold pumerge.
      rewrite E.
      eexists; eauto.
  ...
  -
    ...
    right.
    left.
    destruct (po x1) eqn: E; tryfalse. 
    assert (exists y, pumerge po pi x1 = Some y).
    {
      unfold pumerge.
      rewrite E.
      eexists; eauto.
...
Lemma inos_skip_stmt:
  forall ke ks po pi s, 
    InOS (SKIP, (ke, ks)) (pumerge po pi) ->
    InOS (curs s, (ke, ks)) (pumerge po pi). 
Proof.
  introv Hinsk.
  unfold InOS in *.
  simpljoin.
...
Lemma kscall_kscall_concat: 
  forall ks ks' f s lenv x1 x2 x3 x4, 
    ks ## kcall f s lenv ks' = kcall x1 x2 x3 x4 ->
    ks = kstop \/ exists ks_, ks = kcall x1 x2 x3 ks_. 
Proof.
  induction ks; 
    introv Heq; try (simpl in Heq; false; fail).
  left; auto. 
...
Lemma nca_ena_false:
  forall c pc po pi ip ke ks1 ks s f, 
    no_call_api_cureval c po ->
    no_call_api_scont ks1 po -> 
    enapi (c, (ke, ks1 ## kcall f s empenv ks)) (pc,  (po, pi, ip)) ->
    ((exists vl dl, c = curs (salloc vl dl)) /\ ks1 = kstop).  
Proof.
  introv Hnca Hncs Hena.
  unfolds in Hnca.
  destruct c.
  - 
    simpl in Hena.
    destruct Hena  as [Hf | Hf]; simpljoin; false. 
  -
    unfolds in Hena.
    unfolds in Hnca.
    destruct s0; 
      try (destruct Hena as [Hf | Hf]; simpljoin; tryfalse; fail).
    {
      split.
      - do 2 eexists. eauto.
      -              
        destruct Hena; simpljoin; tryfalse.                     
        lets Hf: kscall_kscall_concat H0; eauto.
        destruct Hf.
        auto.
        simpljoin.
        simpl in Hncs.
...
Lemma SmCTaskSim': 
  forall (pc po pi:progunit) (ip:intunit) (A:osspec) I o O  c ke ks1 ks2 ks ch keh lasrt (tid:tid),
    ...
    (eqdomOS (po,pi,ip) A /\ GoodClient pc po pi /\ goodks (pc,(po,pi,ip)) (ks1##(ks2##ks)) /\
       n_dym_com_int_cd (c,(ke,ks1##(ks2##ks))) /\ good_clt (SKIP, (kenil,ks)) pi /\
       (*good_ret_c (c,(ke,(ks1##(ks2##ks))))*) True) ->
    (
      (~InOS (c,(ke,ks)) (pumerge po pi) /\ 
    ...
    (
      forall f s d1 d2 ft ab tt, 
        ...
        (exists R vlh AC,
            InOS (c,(ke,ks1##kcall f s emp kstop)) (pumerge po pi) /\ ch = curs (hapi_code AC) /\
              Some R = BuildRetA po f (ab,tt) vlh (get_genv (get_smem o)) lasrt tid init_lg /\ no_call_api (c,(ke,ks1)) po /\
...
Proof.
  ...
  {  (* 1st case of the relation construction in the lemma *) 
    lets Hinit:H2 Hc1.
    ...
    rewrite H1.
    apply task_sim.
    { (* 1st clause of TaskSim *) 
      intros.
      rename H11 into Hnena.
      rename H12 into Hntskabt. (* added due to change in def of task sim *) 
      assert (o2=o'').
      {
      ...
      introv Hf. simpl in Hf. destruct A. destruct p; simpljoin.
      eapply cstep_sfe_sfe_false; eauto. 
      ...
      { (* ~sfexec_enapi ... *) 
        introv Hse. apply Hnena.
        unfolds in Hse. unfolds.
        destruct A. destruct p; simpljoin.
        left.
        do 3 eexists; splits; eauto.
        eapply eqdomOS_api_nnone_impl; eauto.
      }
      apply Hinit;auto.
    ...
    { (* 2nd clause of TaskSim *) 
      introv Hsat Hjm Hjo Hlp Hea.
      false.
      eapply inos_enapi_false; eauto. 
    }
    { (* 3nd clause of TaskSim *)      
      intros.
    ...
    (* END 1st case of the relation construction in the lemma *) 
  }
  (*---------------------------------------------*)

  destruct Hc2 as [Hc|Hc2].
  {
    (* 2nd case of the relation construction in the lemma *) 
    ...
    - (* ~ (tlmatch (rev tl) d1 /\ dl_vl_match d1 (rev vl) = true) *) 
      apply task_sim;intros;
        ...
        try (match goal with H: ~ enapi _ _ |- _ => rename H into Hnea end); 
        tryfalse.
      ...
      }
      {
        ...
        assert (exists ab tl tp, B f = Some (ab,(tp,tl)) /\ tlmatch tl d1).  
        {          
    ...
    - (* tlmatch (rev tl) d1 /\ dl_vl_match d1 (rev vl) = true *) 
      ...
      { (* 1st clause of TaskSim *)  
        ...
        do 3 eexists; split; eauto.
        split; auto. 
        { introv Hf. rewrite Hpf in Hf. false. }
      }
      { (* 2nd clause of TaskSim *)  
        intros.
        rename H11 into Hea.
        (* rename H12 into Hntskabt. *)
        split.
        - do 5 eexists; eauto.
        - exists (None: option vallist). 
          split.
          left; auto.
          introv Heqv.
          inverts Heqv. 
          destruct H11 as (_ & Hlb').
          2: { simpljoin. false. }
          subst.
          introv Hoptsp.
          inverts Hoptsp.
          ...
          simpljoin.
          inversion H10; tryfalse.
          subst cst cst' p t C tst C' cst'0 o''.
          { (* client step -- impossible *) 
            destruct H13.
            unfold InOS.
          ...
          inversion H11.
          subst tst C' t p pc0 po0 pi0 ip0 cst' tst'.    
          destruct o as (a&aux).
          ...
          subst o2.
          exists (genv,(emp:env),M1,ir,aux) Ms O Os.
          inversion H12; tryfalse. 
          {
            mttac cstep ltac: (fun H=> inverts H).
            -
              mttac estep ltac: (fun H=> inverts H; tryfalse). 
            -
              mttac sstep ltac: (fun H=> inverts H; tryfalse). 
              subst C0.
              inversion H24. 
              subst.
              match goal with H: (_, _, _) = (_, _, _) |- _ => inverts H end.
              mttac (cst = cst) ltac:(fun H=> clear H).
              splits;auto.
              unfold joinm2.
              exists (ge, (emp:env), M2, ir, aux).
              split;unfold joinmem.
              (exists ge (emp:env) M1 M2 ir aux;splits;auto).
              (exists ge (emp:env) M2 M ir aux;splits;auto).

              eapply inv_substask_emple;eauto.
              eapply CurLINV_ignore_int;eauto.

              assert (kcall f0 s0 le ks0 = kstop## (kcall f0 s0 le kstop ## ks0)). 
              { (simpl;auto). }
              rewrite H8.
              assert (po f0 = Some (ft,d1,d2,s));auto.
              apply pumerge_get with (pi:=pi) in Hpf.
              rewrite Hpf in H27. 
              inversion H27; subst t d0 d3 s0.
              assert (InitTaskSt lasrt tid (ge, le, M1, ir, aux, O)). 
              { (eapply H3;eauto). }
              assert (snd (fst (get_smem (ge, le, M1, ir, aux)))=emp).
              { 
                eapply init_emple.
                eauto.
              }
              ...
              apply CIH;auto;intros;tryfalse.
              splits;auto.
              assert (kstop ## (kcall f0 s empenv kstop ## ks0) = kcall f0 s empenv ks0).
              { simpl;auto. }
              rewrite H17.
              simpl in Hgoodks.
              eapply ltstep_goodks;eauto.
              ...
              right.
              left.
              exists f0 vl0  (revlcons d1 d2) vl0.
              exists s d1 d2 ft tl0.
              (splits;auto).
              inversion H20.
              subst f0 vlh tl0.
              rewrite H17 in H15.
              inversion H15; subst s0 d0 d3 ft0.
              inversion H18; subst vl0 dl.
              splits;auto.
              
              apply good_dl_le_init';auto.
              apply Hgoodpo in H17;auto. 
              destruct H17;auto.
              ...
              intros.
              apply InitAemp;auto.
              destruct (H20 vl0 (revlcons d1 d2)).
              auto.
          }
      }      
      { (* 3rd clause of TaskSim *)   
        intros.
        rename H11 into Hntskabt. (* added due to change in def of task sim *) 
        inversion H10;tryfalse.
        inversion H12.
      ...
      (* END 2nd case of the relation construction in the lemma *) 
  }
  
  (*-------------------------------------*)
  destruct Hc2 as [Hc | Hc2].
  { (* 3rd case of the relation construction in the lemma *) 
    ...
    { (* 1st clause of TaskSim *) 
      intros.
      rename H11 into Hnea.
      false.
      apply Hnea. 
      unfolds.
      right. do 6 eexists; splits; eauto.
      introv Hf. rewrite Hpf in Hf. false.
    }
    { (* 2nd clause of TaskSim *) 
      intros. 
      ...
      destruct Hlstep'.
      { (* step of salloc that does not generate the function body *)
        exists (None: option vallist).
        split. left; auto.
        introv Hleq Hosp.
        inverts Hleq. destruct H12 as (_ (* None = None*) & Hlb'). subst lb'.
        2: { simpljoin; tryfalse. }
        inverts Hosp.
        { simpljoin. false. }
        destruct H12 as (Hch' & Hcste & Hoe & Hne).
        clear Hcste Hne.
        subst OO' Ch'.
      ...
      { (* step of alloc that generates the function body *) 
        exists (Some (nil:vallist)).
        split. right. eexists; eauto.
        introv Hleq Hsp.
        inverts Hsp.
        2: { simpljoin. inverts Hleq; simpljoin; tryfalse. }        
        destruct H12 as (pc0 & A0 & f0 & r & vl0 & tl0 & ks0 & vl'_ & Heq & Hseq & Hch' & Hr & Hlb' & Hasp).
        subst lb'.
        inversion Heq. subst A0 pc0. clear Heq.  
        inversion Hseq. subst f0 vlh tl ks0. clear Hseq. 
        ...
        inverts Hasp.
        match goal with H: (curs _, _) = (curs _, _) |- _ => inverts H end. 
        match goal with H: (curs _, _) = (curs _, _) |- _ => inverts H end.
        eapply list_reverse_prop2 in H17; eauto.
        rewrite rev_involutive in H17; eauto.
        eapply list_reverse_prop2 in H18; eauto.
        rewrite rev_involutive in H18; eauto.
        subst.
        (* rewrite H16. clear H16. *)

        lets Hpp: bp_bpa H0 Hpf Hhf Hbpre Hbp'; eauto.
        apply Hpp in Hpre;eauto.
        instantiate (1:=vl') in Hpre.
        apply Hmsim in Hpre.
        ...
        assert (kcall f0 s empenv ks0 = kstop ## (kcall f0 s empenv kstop ##ks0)).
        {
          simpl;auto.
        } 
        rewrite H11.
        ...
        }
        {
          ...
          inversion H18;subst f0.
          rewrite Hpf in H13;inversion H13;subst ft0 d0 d3 s0.
          simpl in H22. 
          rewrite H22 in Hhf;inversion Hhf;subst ab0.
          exists rf (rev vl) ((ab (vl ++ vl'))).
          splits;auto.
          destruct Hmsim.
  ...
  (*-----------------------------------*)
  destruct Hc2 as [Hc|Hc2].
  { (* 4th case of the relation construction in the lemma *)
    ...
    { (* 1st clause of TaskSim *) 
      ...
      destruct H30.
      eexists;splits;eauto.
      (* END first clause of TaskSim *) 
    }
    
    (*-----------------------------------------------*)
    { (* 2nd clause of TaskSim *) 
      intros.
      specializes H5; eauto.
      destruct H5 as (R & vlh & AC & Hio & Hch & Hr & Hnca & Hsim).
      unfolds in Hnca.
      destruct Hnca as (Hncc & Hnck).
      lets H__: nca_ena_false Hncc Hnck H11; eauto.
      destruct H__ as ((vl_ & dl_ & Hc) & Hks1).
      subst c ks1.
      change (kstop ## kcall f s empenv ks) with (kcall f s empenv ks) in H11.
      simpl in Hio.
      assert (Hken: ke = kenil).
      {
        simpl in H11. destruct H11; simpljoin; eauto. 
      }
      subst ke.
      specialize (Hnalloc vl_ dl_).
      false.
    }
    ...
    (* END 4th case of the relation construction in the lemma*) 
  }
  (*------------------------------------------------*)
  { (* 5th case of the relation construction in the lemma *) 
    ...
    { (* 2nd clause of TaskSim *)
      introv Hsat Hjo2 Hjo Hlsp Hena.
      unfolds in Hena.
      destruct Hena as [Hf | Hf]; simpljoin; tryfalse. 
    }
    { (* 3rd clause of TaskSim *)       
      intros;
        try (match goal with 
    ...
    (* END 5th case of the relation construction in the lemma *) 
  }
...
Ltac invpair := match goal with H: (_, _) = _ |- _ => inverts H end.
Lemma enapi_nea_or_int_false:
  forall pc po pi ip Tl Cl t C' o cst cst' tst', 
    get Tl t = Some Cl -> 
    enapi Cl (pc, (po, pi, ip)) -> 
    ~ enapi_t Tl t (pc, (po, pi, ip)) \/ int_ks (set Tl t C') t -> 
    ltstep (pc, (po, pi, ip)) t Cl cst o C' cst' tst' ->
    False.
Proof.
  introv Hgt Hea Hnea Hsp.
  destruct Hnea as [Hnea | Hintr].
  - apply Hnea. unfolds. eexists; split; eauto.
  - unfolds in Hea.
    destruct Cl. destruct c0.
    destruct Hea as [Hexe | Halc].
    + 
      simpljoin.
      mttac ltstep ltac:(fun H => inverts H).
      mttac cstep ltac:(fun H => inverts H).
      invpair. 
      mttac estep ltac:(fun H => inverts H).
      mttac sstep ltac:(fun H => inverts H); tryfalse. 
      unfolds in Hintr.
      rewrite set_a_get_a in Hintr.
      simpljoin; tryfalse. 
      mttac loststep ltac:(fun H => inverts H); tryfalse. 
      mttac cstep ltac:(fun H => inverts H); tryfalse.
      mttac estep ltac:(fun H => inverts H); tryfalse.
      invpair.
      mttac sstep ltac:(fun H => inverts H); tryfalse. 
      unfolds in Hintr.
...
Proof.
  ...
  unfold tskabsabt.
  destruct Hntabt as [Hl | Hr].
  -
    left.
    simpljoin.
    lets H__: htstepstar_O_local H; eauto.
  ...
  -
    right.
    simpljoin.
    lets H__: htstepevstar_O_local H; eauto.
    specializes H__; eauto.
    simpljoin.
    clear H.
    do 5 eexists; eauto. 
...
Lemma opt_htsp_changeO:
  forall O OO O' pc A cst Ch' cst' lb f vl tl ke ks, 
    opt_htstep (pc, A) (curs (sfexec f vl tl), (ke, ks)) cst O Ch' cst' O' lb ->
    opt_htstep (pc, A) (curs (sfexec f vl tl), (ke, ks)) cst OO Ch' cst' OO lb. 
Proof.
  introv Hosp.
  unfolds in Hosp.
  destruct Hosp as [Ha | Hb].
  -
    simpljoin.
    ...
    apply list_reverse_prop2 in H7.
    rewrite rev_involutive in H7.
    apply list_reverse_prop2 in H8.
    rewrite rev_involutive in H8.
    subst.
    unfolds. left. 
...
Lemma opt_htstep_Oeq:
  forall pc A f vl tl ke ks cst O Ch' cst' O' lb, 
    opt_htstep (pc, A) (curs (sfexec f vl tl), (ke, ks)) cst O Ch' cst' O' lb ->
    O = O'.
Proof.
  introv Hop.
  unfolds in Hop.
  destruct Hop as [Ha | Hb].
  simpljoin.
  inverts H4; auto.
  simpljoin.
Qed.             
    ...
    (
      exists (pc:progunit) (A:osspec) po pi ip Tm To Ol Os Ms Ml I lasrt ,
        no_fun_same po pi/\
          ...
          (forall f t d1 d2 s, po f = Some (t,d1,d2,s) -> good_decllist (revlcons d1 d2) = true /\ GoodStmt' s) /\
          (forall f t d1 d2 s, pi f = Some (t,d1,d2,s) -> good_decllist (revlcons d1 d2) = true /\ GoodStmt' s) /\
          pl = (pc, (po, pi, ip)) /\ ph = (pc, A) /\
          (GoodClient pc po pi /\ True /\ good_t_ks pl Tl /\ good_is_S Sl /\ GoodI I (snd A) lasrt (*/\eqdomcstO cst O*))  
...
Proof.
  ...
  { (* 2nd clause of ProgSim *) 
    intros.
    rename H0 into Hnea.
    rename H1 into Hnprgabt. (* added due to change in def of task sim *) 
    subst.
    ...
    3:apply hp_stepO.
    { (* ~sfexec_enapi \/ ... *)  
      right. unfolds. rewrite set_a_get_a.
      do 5 eexists; eauto.
    }
    ...
    exists
      (ge, (empenv:env), m, isrupd ir i true, (false, i :: si, (nil:cs)))
        (substaskst (ge, (empenv:env), m, isrupd ir i true, (false, i :: si, (nil:cs))) (merge Mc Ml'))
        (merge Mc Ml')
        (merge Oc Ol')
        ((curs s), (kenil, ( kint c ke le ks)))
        (curs (hapi_code (x)), (kenil, kevent ch keh ksh)). 
    splits.
    ...
    rewrite set_a_get_a' in H11; eauto. 
    ...
    rename H16 into Hnodead.
    assert (t'0<>t');auto.

    apply Hrsim with (Ch:=Ch0) in H14.

    destruct H14 as (or&M'r&Clr&O'r&Htmr&Htor&Hprojr&Hrlinv&Htasksr&Hsimr).
    ...
    { (* ~enapi *)
      introv Hnea'.
      eapply enapi_nea_or_int_false; eauto. 
    }
  ...
  (*-----------------------------switch step ---------------------------*)
    ...
    eapply H11 with (OO:=merge Oc Os) (Ms:=Ms) (Os:=Os) in H16.
    destruct H16 as (Ch'&sleft&k&OO'&Mc'&ol&O'&Os'&Olc&Occ&Hhch&Hhtstep&Hljoin&Hhjoin&Hhjoin'&Hinv'&Hswinv&Hlinv'&H16).
    destruct H16.
    destruct H16 as (Hswpre&H16).
    ...
    assert (hpstep (pc, A) (set Th t (curs (hapi_code (sched;; sleft)), k)) cst' x0 
              (TasksMod.set Th t (curs (hapi_code sleft), k)) cst' (set x0 curtid (oscurt t')) /\
              forall ab, (tst, x0 , ab) |= AHprio (snd A) t' ** Atrue). 
    { 
      eapply sw_same_t with (l:=l) (tp:=tp) (x:=x) (tst:=tst) (t':=t');eauto.
      unfolds in Hgoodi.
      mytac;simpl;auto.

      apply htstepstar_tidsame in H17.
      unfolds in H17.
      rewrite <- H17.
      auto.

      rewrite H2 in Hproj;inverts Hproj.
      rewrite H18 in H2;inverts H2.
      simpl snd.
              ...
              (set Th t (curs (hapi_code sleft), k)) cst' (set x0 curtid (oscurt t')) ) as Hpstepstar.
    {
                ...
                (set Th t (curs (hapi_code sleft), k)) cst' (set x0 curtid (oscurt t')) ) as Hhpstep.
      {
        ...
        lets H__: htstepstar_tidsame H17; eauto.
        eapply hpstep_star with (T':=(set Th t (curs (hapi_code  (sched;; sleft)), k))) (cst':=cst') (O':=x0) (t:=t); eauto.
        destruct H0;auto.
        { (* get _ curtid = _ *)
        ...
        { (* ~sfexec_enapi *) 
          introv Hf.
          unfolds in Hf. destruct k. destruct A. destruct p.
          simpljoin.
    ...
    destructs H0.
    simpl in H20.
    inversion H20;subst x2 x7 x3 x6 x5.
    subst ol.
    ...
    lets Hnmc:join_store' H22 Hmc'n.
    ...
    eapply partm_task_get_set;eauto.
    apply htstepstar_tidsame in H17.
    unfold tidsame in H17.
    unfold get in H17,Hosabsttid.
    simpl in H17.
    simpl in Hosabsttid.
    rewrite Hosabsttid in H17.
    unfold tid in *.
    rewrite <- Ho'set.
    ...
    unfold substaskst.
    auto.
    rewrite set_a_get_a' in H23; eauto.
    eapply htstepstar_nodead_still;eauto.
    clear -H24.
    unfolds.
    unfolds in H24.
    ...
    Lemma abst_set_get_neq:
      forall id1 id2 y O,
        id1<>id2 ->
        OSAbstMod.get (OSAbstMod.set O id2 y) id1 = OSAbstMod.get O id1. 
    Proof.
      intros.
    ...
    destruct H20 as (Mcc&Hmcc).


    eapply swi_rdy_inv'''' with
      (o:=o) (Ol:=Occ) (Os:=Os') (OO:=merge Occ Os') (I:=I) (t:=t) (t':=t')
      (S:=(ge, les, m, ir, au)) (o':=on) (tp:=tp) (Mcc:=Mcc) (b:=b) (sc:=snd A) (li:=lasrt) in H0;auto. 
    ...
    lets Hsubtrans: sub_trans H20 H22.
    splits;auto.
    splits;auto.
    splits;auto.
    eapply lpstep_goodks;eauto.
    ...
    eapply mem_join_merge_minus_join_store' with (Ms:=Ms) (Mc:=Mc') (Mcc:=Mcc) (Mc':=Mc'0) (m':=m');
      eauto. 
    ...
    destruct H25 as (Hrinv&Hrdisj&Hodisj).
    ...
    apply H16 with (o':=substaskst o (merge Mr (minus Mc Mc'))) (O'':= merge Or Olc) in H25.
    ...
    rewrite set_a_get_a' in H24; eauto.
    eapply htstepstar_nodead_still;eauto.
    clear -Hdead.
    unfolds.
    unfolds in Hdead.
    ...
    2: {
      ...
      rewrite repl_change_tstm_trans.
      mytac.
      rewrite H17 in Hproj;inverts Hproj;auto.
    }
    2: { 
      destruct o as [[[[]]]].
      unfold getmem;simpl.
              ...
              forall ab, (tst, x0 , ab) |= AHprio (snd A) t' ** Atrue).
    {
      ...
      unfold getsched in H16.
      simpl snd in H16.
      unfold satp in *.
  
      unfold SWPRE_DEAD in H16.
              ...
              (set Th t (curs (hapi_code sleft), k)) cst' (set x0 curtid (oscurt t')) ) as Hpstepstar.
    {
                ...
                (set Th t (curs (hapi_code sleft), k)) cst' (set x0 curtid (oscurt t')) ) as Hhpstep.
      {
        ...
        { (* ~sfexec_enapi *) 
          introv Hf.
          unfolds in Hf. destruct k. destruct A. destruct p. simpljoin; tryfalse. 
        }
      }
      auto.
    }
    ...
    destruct HHHH as (Hxx&HHHH).
    clear Hxx.
    assert (forall aop,  (substaskst o Mc', Occ, aop) |= SWPRE (snd A) x t).
    { 
      intros.
      lets Hx:H16 aop.
      destruct Hx;auto.
    ...
    destruct H22 as (Mcc&Hmcc).
  

    lets Hx:aux_atrue Hlinv'.
    destruct Hx as (o1&M2&O1&O2&Hjoinf&Hjoinof&Hlinvf).

    eapply swi_rdy_inv_dead with
      (o:=o) (Ol:=Occ) (Os:=Os') (OO:=merge O' Os') (I:=I) (t:=t) (t':=t')
      (S:=(ge, les, m, ir, au)) (Ms:=Ms) (o':=on) (tp:=tp) (Mcc:=Mcc) (b:=b) (sc:=snd A) (o1:=o1)
      (M2:=M2) (O1:=O1) (O2:=O2) (li:=lasrt) in H0; eauto.  
    ...
    lets Hsubtrans: sub_trans H22 H23.
    splits;auto.
    splits;auto.
    splits;auto.
    eapply lpstep_goodks;eauto.
...
(* ** ac:   Check  store_sub_minus_eq. *)
  ...
  *)
    ...
    destruct H27 as (M''&Hm''merge&Htsim).
    rewrite Hm''merge in Htsim.
    assert (RdyChange (substaskst on (merge Mc'0 Mn))=substaskst on (merge Mc'0 Mn)).
    eapply rdyinv_isremp with (I:=I) (O:=Oc'0) (M:=Mc'0);eauto.
    rewrite H27 in Htsim.
    ...
    { 
      ...
      rewrite set_a_get_a' in H25; eauto.
      eapply htstepstar_nodead_still;eauto.
      clear -Hdead.
      unfolds.
      unfolds in Hdead.
    ...
    auto.
    eapply htstepstar_nodead_still;eauto.
    eapply AHprio_local with (O:=x0) in H20.
    eapply ahprio_nodead;eauto.
    clear -Hhjoin Hhjoin' H19.
    unfolddef.
    unfolddef.
  ...
  (*-----------------------------stkinit step--------------------------------------*)
    ...
    simpl in Hsubsub.
    subst o1'.
    eapply H14 with (Ms:=Ms) (Os:=Os) (OO:=merge Oc Os)in H16;eauto.
    2: { 
      unfold getmem;simpl.
      eapply part_disjm with (M:=Ml) (T:=Tl) (Tm:=Tm) (t:=t);eauto.
      clear -Hmjoin.
    ...
    destruct H16 as (Ch'&v11&v12&t'&p&sh&k&OO'&OO''&ol&Mcre&O'&Os'&Olc&Ocre&H16).
    mytac.
    subst.
    destruct Sl as [[[[]]]].
    simpl in Hproj.
    ...
    destruct H28.
    simpl in H16.
    lets Hx:evalval_mono H28 H16.
    rewrite Hx in *.
    rewrite H16 in H5;inverts H5.
    clear Hx.
    simpl in H17.
    lets Hx:evalval_mono H28 H17.
    rewrite Hx in *.
    rewrite H17 in H6;inverts H6.
    clear Hx.
    simpl in H18.
    lets Hx:evalval_mono H28 H18.
    rewrite Hx in *.
    rewrite H18 in H7;inverts H7.
    clear Hx.
    eapply htstepstar_O_local with (Of:= (minus Ol Oc)) (OO:=O) (cst:=(ge, cenvs, M)) in H19.
    destruct H19.
    ...
    inverts H20.
    mytac.
    assert (hpstepstar  (pc, A) Th (ge, cenvs, M) O
              (set (set Th t (curs (hapi_code  sh), (keh,ksh))) t'0 (nilcont s))
              (ge, set cenvs t'0 le, M') (merge (set OO' abtcblsid (abstcblist x2)) (minus Ol Oc))).
    {
      eapply hpstep_star;eauto.
      assert ((set Th t (curs (hapi_code sh), (keh,ksh))) =
                set (set Th t (curs (hapi_code (spec_crt v1 v (Vint32 p);; sh)), (keh,ksh))) t (curs (hapi_code sh), (keh,ksh))). 
      {
        apply set_set_eq.
      }
      rewrite H20.
      ...
      { (* ~sfexec_enapi *) 
        introv Hf.
        unfolds in Hf.
        destruct A. destruct p0. simpljoin; tryfalse. 
      }
    ...
    do 6 destruct H21.
    destructs H21.
    inversion H30;subst x3 x4 x6 x7 x8;clear H30.
    inversion H21;subst e4 e5 x5 i l0;clear H21.
    rename m0 into Mlc.
    exists pc A po pi ip.
    rename t'0 into t'.
    ...
    simpl substaskst.
    eapply linv_change_linv_aux with (e':=empenv) (aux':=(true,nil,nil)) (ir':=ir) in H26;eauto.
    clear -H26.
    unfold LINV.
    exists init_lg.
    unfolds in H26.
    ...
    eexists;split;eauto.
    simpl RdyChange.
    rewrite map_get_set in H30;inverts H30.
    eapply SmCTaskSim;eauto.
    unfolds.
    simpl.
    destructs H33.
    simpl RdyChange in H30.
    ...
    (*-----------other tasks-------------*)
    rewrite map_get_set' in H30;auto.
    rewrite map_get_set' in H30;auto.
    assert (t'0<>t) by auto.
    eapply Hrsim in H21;eauto.
    mytac.
    exists x3 x4 x5 x6.
    splits;auto.
    eapply TMSpecMod.mapsto_mapsto_put;eauto.
    ...
    apply hpstep_eqdomto in H20;auto.
    eapply join_merge_minus;eauto.
    eapply parto_sub;eauto.
    ...
    simpl in Hsubsub.
    subst o1'.
    eapply H13 with (Ms:=Ms) (Os:=Os) (OO:=merge Oc Os) in H15;eauto.
    2: { 
      unfold getmem;simpl.
      eapply part_disjm with (M:=Ml) (T:=Tl) (Tm:=Tm) (t:=t);eauto.
      clear -Hmjoin.
    ...
    assert (hpstepstar  (pc, A) Th (ge, cenvs, M) O
              (set (set Th t (curs (hapi_code  sh), (keh,ksh))) t'0 (nilcont (sskip None))) (ge, cenvs, M)
              (merge (set OO' abtcblsid (abstcblist x2)) (minus Ol Oc)) ). 
    { 
      eapply hpstep_star;eauto.
      assert ((set Th t (curs (hapi_code sh), (keh,ksh))) =
                set (set Th t (curs (hapi_code (spec_crt v1 v (Vint32 p);; sh)), (keh,ksh))) t (curs (hapi_code sh), (keh,ksh))). 
    ...
    eapply joinsig_indom_neq;eauto.
    apply H31;rewrite map_get_set;auto.
    ...
    simpl in Hsubsub.
    subst o1'.
    eapply H9 with (Ms:=Ms) (Os:=Os) (OO:=merge Oc Os) in H10;eauto.
    2: {
      unfold getmem;simpl.
      eapply part_disjm with (M:=Ml) (T:=Tl) (Tm:=Tm) (t:=t);eauto.
      clear -Hmjoin.
    ...
    destruct H10 as (Ch'&p&sh&k&t'&OO'&OO''&O'&Os'&H10).
    mytac.
    subst.
    destruct S' as [[[[]]]].
    simpl in Hproj.
    ...
    destruct H13.
    simpl in H10.
    lets Hx:evalval_mono H13 H10.
    rewrite Hx in *.
    rewrite H7 in H10;inverts H10.
    clear Hx.

    eapply htstepstar_O_local with (Of:= (minus Ol Oc)) (OO:=O) (cst:=(ge, cenvs, M)) in H11.
    destruct H11.
    destruct H10 as (Hhtstepstar&Hjoin').
    assert (htstepstar (pc, A) t Ch (ge, cenvs, M) O
    ...
    inverts H16;mytac.
    assert (hpstepstar  (pc, A) Th (ge, cenvs, M) O
              (set Th t (curs (hapi_code sh), (keh,ksh))) (ge, del cenvs t, M)
              (merge (set OO' abtcblsid (abstcblist x2)) (minus Ol Oc)) ).
    {
      eapply hpstep_star;eauto.
    ...
    splits;auto.
    intros.
    rewrite map_get_set' in H21;auto.
    apply Hrsim in H21;auto.
    mytac.
              ...
              (merge (set OO' abtcblsid (abstcblist x2)) (minus Ol Oc)) ). 
    {
      ...
      { (* ~sfexec_enapi *)
        introv Hf. unfolds in Hf. destruct A. destruct p0. simpljoin; tryfalse.
      }
    }
    do 2 eexists;split;eauto.
    assert (merge (set OO' abtcblsid (abstcblist x2)) (minus Ol Oc) = set x0 abtcblsid (abstcblist x2)).
    ...
    { 
      ...
      unfolds.
      intros.
      unfolddef;rewrite H in H17;inverts H17.
      unfolds.
      unfolds in H18.
      eexists.
      eapply join_get_get_l;eauto.
    ...
    assert (exists Ch,get Th t' = Some Ch).
    {
      ...
      eapply join_get_get_l in H17;eauto.
      clear -H18 H17 Heqdomto H16.
      unfolds in Heqdomto.
      mytac.
      unfolddef.
      auto.
      rewrite H17 in H;inverts H.
      assert (get x t' = Some (p,x3)).
      {
        unfolds in H18.
        eapply join_get_get_l in H18;eauto.
        apply map_get_sig.
      }
    ...
    exists (e2,e1,m0,i,au) (e2,e1,(merge Mc Mdc),i,au) (merge Mc Mdc) (merge O' Odc)
      (SKIP , (kenil, ks)) (curs (hapi_code sh), (keh, ksh)).
    splits;auto.
    eapply TMSpecMod.mapsto_mapsto_put;eauto.
    eapply TOSpecMod.mapsto_mapsto_put;eauto.
    unfolds.
    ...
    destruct H35.
    subst t'0.
    clear -H18 H34 H17 Hjoin'.
    unfolds in H34.
    rewrite map_get_set in H34;auto.
    assert ( Some (abstcblist x2) = Some (abstcblist x2) ) by auto.
    apply H34 in H.
    unfolds in H18.
    eapply map_join_get_no_perm1 in H18;eauto.
    unfolds in H.
    mytac.
    unfolddef.
    rewrite e in H18.
    tryfalse.
    apply map_get_sig.
    rewrite map_get_set' in H33;eauto.
    assert (task_no_dead O t'0).
    { 
      eapply htstepstar_nodead_still;eauto.
      
      eapply join_get_get_l in H17;eauto.
      clear -H34 H35 H18 H17.
      unfold task_no_dead in *.
      intros.
      unfolddef.
      rewrite H in H17;inverts H17.
      rewrite map_get_set in H34.
      assert (Some (abstcblist x2) = Some (abstcblist x2)) by auto.
      apply H34 in H0.
      unfold indom in *.
      mytac.
    ...
    lets Hx: Hrsim H32 H33 H36.
    mytac.
    exists x6 x7 x8 x9; splits; eauto.
    eapply TMSpecMod.mapsto_mapsto_put;eauto.
    eapply TMSpecMod.mapsto_mapsto_put;eauto.
    eapply TOSpecMod.mapsto_mapsto_put;eauto.
    eapply TOSpecMod.mapsto_mapsto_put;eauto.
    ...
    (* END 2nd clause of ProgSim *)  
  }
  
  { (* 3rd clause of ProgSim *) 
    intros.
    destruct H0 as (Hea & Hnint).
    subst.
    unfolds in Hea.
    destruct Hea as (C & Hgc & Hena).
    unfolds in Hena.
    destruct C.
    destruct c0.

    destruct Hena as [Hexe | Halc].
    -
      destruct Hexe as (f & vl & tl & Hc & Hpo & He).
      subst c e.
      inversion H;tryfalse.      
      { (* interrupt step -- impossible *) 
        subst.
        mttac lintstep ltac:(fun H=> inverts H).
        false.
        mttac int_ks ltac:(fun H=> apply H).
        unfolds.
        rewrite set_a_get_a.
        do 5 eexists; eauto. 
      }
      {
        ...
        rewrite <- H2 in Hinv.
        unfold satp in *.
        apply H1 with (Cl':=C') (cst:=cst) (cst':=cst') (o'':=tst')
                      (o2:=o) (Mf:=minus Ml Mc) (OO:=merge Oc Os) in Hinv; auto.
        4: {
          rewrite Hgc in Htaskscl. inverts Htaskscl.
          unfolds. left. do 3 eexists; splits; eauto.
        }
        
        destruct Hinv as (HCh & Hinv').
        destruct Hinv' as (lb & Hlb & Ha).
        exists lb.
        split; auto.
        introv Hlbe Hopsp.
        unfolds in Hopsp.
        destruct Hopsp as (Ch_ & Ch' & HgCh & HTh' & Hotsp).
        rewrite Htasksch in HgCh.
        inverts HgCh.
        destruct HCh as (f_ & vl_ & tl_ & ke & ks & HCh).
        subst Ch_.       

        lets Hotsp': opt_htsp_changeO (merge Oc Os) Hotsp; eauto.
        lets H__: Ha Hlbe Hotsp'; eauto.
        destruct H__ as (o' & Ms' & O'_ & Os' & Hjoin2l & Hjoinh & Hinv & Htlinv & Htsim).
        (* destruct Hinv as (Ch'_&OO'&o'&Ms'&O'_&Os'&Hhtstep&Hjoin2l&Hjoinh&Hinv&Htlinv&Htsim). *)

        lets Hoeq: opt_htstep_Oeq Hotsp; eauto. 
        subst O'.
        rename O'_ into O'.
        remember (merge Oc Os) as OO'.
        assert (Hjo_: join OO' (minus Ol Oc) O).
        {
          rewrite HeqOO'.
          eapply join_merge_minus; eauto.
          eapply parto_sub; eauto. 
...

        {
          ...
          exists (TOSpecMod.put To t' O') (minus O Os') Os' Ms' (minus (get_mem (get_smem tst')) Ms').
          exists I lasrt.
          ...
          exists (e, e0, m, i, l) o' (get_mem (get_smem o')) O' C' Ch'.
          splits;auto.
          clear -Hjoin2l.
          unfolds in Hjoin2l.
          mytac.
          ...
          rename H14 into Hnodead.
          assert (t'0<>t');auto.

          apply Hrsim with (Ch:=Ch) in H12.

          destruct H12 as (or&M'r&Clr&O'r&Htmr&Htor&Hprojr&Hrlinv&Htasksr&Hsimr).
          ...
          auto. 
          (* eapply hpstep_eqdomto with (T:=Th) (O:=O) (cst:=cst) (cst':=cst') (p:=(pc,A));eauto. *)
          eapply eqdomTO_setT with (T:=Th); eauto.
        }
    ...
    -
      destruct Halc as (vl & dl & f & s0 & E & ks' & Hc & Hs & Hpo & He).
      subst c s e.
      inversion H; tryfalse.
      {
        subst.
        rewrite H1 in Hgc. inverts Hgc.
        mttac lintstep ltac:(fun H=> inverts H).
        false.
        mttac int_ks ltac:(fun H=> apply H).
        unfolds.
      ...
      }
      {
        ...
        4: {
          rewrite Hgc in Htaskscl. inverts Htaskscl.
          unfolds. right. do 6 eexists; splits; eauto.
        }
      ...
      (* END 3rd clause of ProgSim *)
  }

  { (* 4th clause of ProgSim *) 
    (*------------------------------event case----------------------------------------*)
    intros.
    ...
    eapply H4 with(cst:=cst') (cst':=cst') (Cl':=C') (Mf:=minus Ml Mc)
    ...
    exists (e, e0, m, i, l) o' (get_mem (get_smem o')) Oc' C' Ch'.    
    splits;auto.
    clear -Hjoin2.
    unfolds in Hjoin2.
    mytac.
    ...
    rename H13 into Hnodead.
    assert (t'<>t);auto.

    apply Hrsim with (Ch:=Ch0) in H11.

    destruct H11 as (or&M'r&Clr&O'r&Htmr&Htor&Hprojr&Hrlinv&Htasksr&Hsimr).
    ...
    eapply join_merge23'; eauto.
    apply join_comm; auto.
    (* END 4th clause of ProgSim *) 
  }

  { (* 5th clause of ProgSim *) 
    (*-----------------------abort case---------------------------*)
    intros.
    ...
    apply H11 with (cst:=cst)(Os:=Os) (Ms:=Ms) (o':=o) (Mf:=minus Ml Mc) (OO:=merge Oc Os) (OOO:=O) (Of:=minus Ol Oc) in H3;auto.
    
    apply  hp_stepstarabt.
    inversion H3.
    subst.
    destruct H14 as (c'&cst'&O'&Htstepstar&Htstepabt).
    ...
    mytac.
    rewrite H14 in Hproj;inverts Hproj.
    unfolds.
    ...
    rewrite H14 in Hmjoin;simpl in Hmjoin;auto.
    ...
    unfolddef.
    rewrite Htl in H0;inverts H0.
    eapply H8 with (Ms:=Ms) (Os:=Os) (OO:=merge Oc Os) in H10.
    ...
    unfolddef.
    rewrite Htl in H0;inverts H0.
    eapply H9 with (Ms:=Ms) (Os:=Os) (OO:=merge Oc Os) in H10.
    ...
    unfolddef.
    rewrite Htl in H0;inverts H0.
    eapply H5 with (Ms:=Ms) (Os:=Os) (OO:=merge Oc Os) in H10.
    simpl substaskst in *.
    simpl get_smem in *.
    mytac.
    destruct H3.
...
Lemma dladd_revlcons_eq: forall d1 d2, inferules.dladd d1 d2 = revlcons d1 d2.
Proof.
  intros.
...
Definition OSCorrect osc A init := 
    (
      forall (pc: progunit) O cst T S t,
        get O curtid = Some (oscurt t) -> 
        InitTasks T pc cst O ->
        init S O ->
        WellClient pc (fst (fst osc)) (snd (fst osc)) ->
        indom T t -> 
        oscorrectness.prog_sim (pc, osc, T, cst, S, t) (pc, A, T, cst, O)
    ).
