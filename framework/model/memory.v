...
Program Instance memMap: PermMap addr (bool * memval) mem :=
  {
    ...
    map_dec_a := HalfPermMap.dec_A
  }.
Next Obligation.
  apply HalfPermMap.map_same; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_same'; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_join_emp; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_join_pos; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_join_comm; auto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_join_assoc; eauto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_join_cancel; eauto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_join_deter; eauto.
Qed.
Next Obligation.
  apply HalfPermMap.map_sv_ref; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_sv_comm; auto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_sv_assoc; eauto.
Qed.
Next Obligation.
  apply HalfPermMap.map_perm_eql; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_flip_sv; auto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_eql; eauto.
Qed.
Next Obligation.
  apply HalfPermMap.map_disjoint; auto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_get_unique; eauto.
Qed.
Next Obligation.
  apply HalfPermMap.map_get_sig; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_get_sig'; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_get_set; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_get_set'; auto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_join_get_none; eauto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_join_getp_some; eauto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_set_sig.
Qed.
Next Obligation.
  apply HalfPermMap.map_join_set_none; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_join_set_perm; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_join_get_sig; auto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_join_get_sig_perm; eauto.
  Qed.
Next Obligation.
  apply HalfPermMap.map_merge_semp; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_join_merge; eauto.
Qed.
Next Obligation.
  apply HalfPermMap.map_merge_comm; eauto.
Qed.
Next Obligation.
  apply HalfPermMap.map_disjoint_semp; auto.
  Qed.
Next Obligation.
  apply HalfPermMap.map_disjoint_semp'; eauto.
Qed.
Next Obligation.
  apply HalfPermMap.map_minus_semp; auto.
Qed.
Next Obligation.
  apply HalfPermMap.map_join_minus; eauto.
Qed.
Next Obligation.
  eapply HalfPermMap.map_del_sem.
Qed.
...
Definition cmpaddr :=
  fun (c : comparison) (x y : addrval) =>
    match x with
      (b1, o1) =>
        match y with
          (b2, o2) => 
            match c with
            | Ceq => (b1 =? b2) && Int.eq o1 o2
            | Cne => negb ((b1 =? b2) && (Int.eq o1 o2))
            | Clt => (b1 <? b2) || ((b1 =? b2) && Int.ltu o1 o2)
            | Cle => (b1 <? b2) || ((b1 =? b2) && Int.ltu o1 o2) || ((b1 =? b2) && (Int.eq o1 o2))
            | Cgt => (b2 <? b1) || ((b1 =? b2) && Int.ltu o2 o1) 
            | Cge => (b2 <? b1) || ((b1 =? b2) && Int.ltu o2 o1) || ((b1 =? b2) && (Int.eq o1 o2))
            end
        end
    end.
...
Definition cmp_val (c: comparison) (v1 v2: val): option val :=
  match v1, v2 with
  | Vint32 n1, Vint32 n2 =>
      if (cmpu c n1 n2) then 
        Some(Vint32 (Int.one))
      else Some(Vint32 (Int.zero)) 
  | Vptr a1, Vptr a2 =>
      if (cmpaddr c a1 a2) then
        Some (Vint32 (Int.one))
      else Some (Vint32 (Int.zero)) 
  | Vnull, Vptr a =>
      if (cmpaddr c (xH, Int.repr 0) a) then
        Some (Vint32 (Int.one))
      else Some (Vint32 (Int.zero)) 
  | Vptr a, Vnull =>
      if (cmpaddr c a (xH, Int.repr 0)) then
        Some (Vint32 (Int.one))
      else Some (Vint32 (Int.zero)) 
  | Vnull, Vnull => 
      if (cmpaddr c (xH, Int.repr 0) (xH, Int.repr 0)) then
        Some (Vint32 (Int.one))
      else Some (Vint32 (Int.zero)) 
  | _, _ => None 
  end.
...
Definition bop_type (t1 t2:type) (op1:bop):=
  ...
  | olt => match t1 with
           (* added to support pointer comparison *)
           | Tptr t => match t2 with
                       | Tptr t' => Some Tint32
                       | Tnull => Some Tint32
                       | Tcom_ptr x => Some Tint32 
...
Program Instance EnvMap: PermMap ident (block * type) EnvMod.map :=
  {
    ...
    merge := EnvMod.merge;
    minus := EnvMod.minus;
    map_dec_a := EnvMod.map_dec_a
}.
Next Obligation.
  apply EnvMod.map_join_emp.
Qed.
Next Obligation.
  apply EnvMod.map_join_pos; auto.
Qed.
Next Obligation.
  apply EnvMod.map_join_comm; auto.
Qed.
Next Obligation.
  eapply EnvMod.map_join_assoc; eauto.
Qed.
Next Obligation.
  eapply EnvMod.map_join_cancel; eauto.
Qed.
Next Obligation.
  eapply EnvMod.map_join_deter; eauto.
Qed.
Next Obligation.
  apply EnvMod.map_eql; auto.
Qed.
Next Obligation.
  apply EnvMod.map_disjoint; auto.
Qed.
Next Obligation.
  eapply EnvMod.map_get_unique; eauto.
Qed.
Next Obligation.
  apply EnvMod.map_get_sig.
Qed.
Next Obligation.
  apply EnvMod.map_get_sig'; auto.
Qed.
Next Obligation.
  apply EnvMod.map_get_set.
Qed.
Next Obligation.
  apply EnvMod.map_get_set'; auto.
Qed.
Next Obligation.
  eapply EnvMod.map_join_get_none; eauto.
Qed.
Next Obligation.
  eapply EnvMod.map_join_get_some; eauto.
Qed.
Next Obligation.
  apply EnvMod.map_set_emp.
Qed.
Next Obligation.
  apply EnvMod.map_set_sig.
Qed.
Next Obligation.
  apply EnvMod.map_join_set_none; auto.
Qed.
Next Obligation.
  apply EnvMod.map_join_get_sig; auto.
Qed.
Next Obligation.
  apply EnvMod.map_merge_sem; auto.
Qed.
Next Obligation.
  eapply EnvMod.map_join_merge; eauto.
Qed.
Next Obligation.
  eapply EnvMod.map_merge_comm; eauto.
Qed.
Next Obligation.
  apply EnvMod.map_minus_sem; auto.
Qed.
Next Obligation.
  eapply EnvMod.map_join_minus; eauto.
Qed.
Next Obligation.
  apply EnvMod.map_del_sem.
Qed.
