...
Definition sfexec_enapi (C: code) (p: hprog) :=
  match C with
    (c, (ke, ks)) =>
      match p with
        (_, (ospc, _, _)) => 
         ke = kenil /\ (exists f vl tl, c = curs (sfexec f vl tl) /\  ospc f <> None)
      end
  end.
  
(* the following definition is used in task simulation and related proofs *) 
...
| hpcrt_step : forall (T:tasks) (O O':osabst) (t t':tid)
    ...
    join tls (sig t' (v3,rdy)) tls' ->
    
    hpstep (pc, A) T cst O (set (set T t C') t' (nilcont s)) cst' O'
...
| hpcrtskip_step : forall (T:tasks) (O O':osabst) (t t':tid)
    ...
    set O abstcblsid (abstcblist tls') = O' ->
    join tls (sig t' (v3,rdy)) tls' ->      
    hpstep (pc, A) T cst O (set (set T t C') t' (nilcont (sskip None))) cst O'
           
| hpdel_step: forall p T T' t t' C C' sdel ke ks ge cenvs M O O' tls tls' cst cst' prio st,
...
Definition evt_ks T t := exists c ke c' ke' ks', get T t = Some (c, (ke, kevent c' ke' ks')).  
...
| hp_stepS: forall p T T' T'' cst cst' cst'' O O' O'' t C,
    get O curtid = Some (oscurt t) ->
    get T t = Some C ->
    ((~sfexec_enapi C p) \/ evt_ks T' t) ->  
    hpstep p T cst O T' cst' O' ->
    hpstepstar p T' cst' O'  T'' cst'' O''->
    hpstepstar p T cst O T'' cst'' O'' .
