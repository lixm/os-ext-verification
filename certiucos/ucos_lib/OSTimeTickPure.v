...
Lemma tcbdllseg_compose':
  forall (s : RstateOP) (P : asrt) (h hp t1 tn1 t2 tn2 : val) (l1 l2 : list vallist),
...
Lemma neq_null_false_elim:
  forall x v s P, s |= LV x @ OS_TCB ∗ |-> v ** P -> s |= Aisfalse (x ′ !=ₑ NULL) -> v = Vnull. 
Proof.
  intros.
  destruct_s s.
  simpl in H; mytac.
...
Lemma neq_null_true_elim:
  forall x v s P, s |= LV x @ OS_TCB ∗ |-> v ** P -> s |= Aistrue (x ′ !=ₑ NULL) -> v <> Vnull. 
Proof.
  intros.
  intro.
  substs.
...
Lemma z_to_nat_shr3:
  forall x,
    0 <= Int.unsigned x ->
    Int.unsigned x < 64 ->
    (0 <= Z.to_nat (Int.unsigned (Int.shru x ($ 3))) < 8)%nat.  
Proof.
  intros.
...
Proof.
  inductions l; intros.
  simpl in H; mytac.
  unfold TCBNode_P in H2; destruct x5; mytac; auto.
...
Proof.
  ...
  -
    ...
    +
      ...
      unfold V_OSTCBPrio in *;simpl in H;inverts H.
      (* clear H9. *)
...
Lemma tick_rdy_ectr_get_ectr_eq:
 ...
 (exists et etbl, get_ectr (Vptr eid) head l = Some (et, etbl)). 
Proof.
  induction els.
  - 
    intros.
  ...
  -
    ...
    +
      xunfold H2.
      (* rename HeqH2 into HeqH1. *)
      (* ; rename HeqH3 into HeqH2. (* new add *) *)
      ...
      rewrite <- HeqX.
      rewrite <- HeqH2 in *. 
      symmetry in HeqH3. (* change HeqH3 -> HeqH4 *)
      eapply IHels;eauto.
      unfolds.
      instantiate (1:= vl).
      rewrite <- HeqY.
...
Lemma TCBList_P_in_list_get_some_sem :
  forall l v rtbl tcbls i0 i1 i3 i4 i6 i7 i8 i9 i10 a,
    ...
    array_type_vallist_match Int8u rtbl ->
    length rtbl = ∘OS_RDY_TBL_SIZE ->
    exists tid p, TcbMod.get tcbls tid = Some(p, wait a).
Proof.
  inductions l; intros.
  - 
    simpl in H0; tryfalse.
  -
    ...
    substs.
    simpl in H; mytac.
    unfold TCBNode_P in H6; destruct x2; mytac.
    unfolds in H8; mytac.
    unfolds in H10; mytac.
    unfolds in H10.
    unfold WaitTCBblk in H10.
    assert (0 <= Int.unsigned i6 < 64) by auto.
    (* rename H17 into Hlhq; rename H18 into H17. (* new add *) *)
    pose proof prio_rtbl_dec i6 rtbl H12 H2 H3.
    destruct H13.
    unfolds in H8. 
    pose proof H8 i6; clear H8.
    unfold RdyTCBblk in H14.
    unfold V_OSTCBPrio in H14.
    simpl in H14.
    assert (Some (Vint32 i6) = Some (Vint32 i6) /\ prio_in_tbl i6 rtbl) by auto.
    apply H14 in H8.
    destruct H8.
    unfolds in H8; simpl in H8; tryfalse.
    
    exists x.
    pose proof H5 x.
    rewrite TcbMod.get_sig_some in H14.
    destruct (get x1 x); tryfalse.
    destruct (get tcbls x); tryfalse.
    substs.
    (* unfold V_OSTCBPrio in H14. *)
...
Proof.
     ...
     (* \/ i5 = $ OS_STAT_POSTQ *)).
  {
    ...
    mytac; unfolds in H2; destruct x0; mytac.
    mttac R_TCB_Status_P ltac:(fun H => unfolds in H; mytac).
    mttac RL_TCBblk_P ltac:(fun H => unfolds in H; mytac).
  ...
  - 
    ...
    +   
      ...
      mttac RL_TCBblk_P ltac: (fun H => unfolds in H; mytac).
      mttac V_OSTCBStat ltac: (fun H => unfolds in H; simpl in H; inverts H).
      mttac V_OSTCBEventPtr ltac: (fun H => unfolds in H; simpl in H; inverts H). 
      assert(Heq: $ OS_STAT_RDY = $ OS_STAT_RDY) by auto.
      apply H26 in Heq; tryfalse. 
    +
      eapply in_TCBList_P_TCBNode_P in H2; eauto.
      mytac; unfolds in H2; destruct x0; mytac.
      mttac R_TCB_Status_P ltac: (fun H => unfolds in H; mytac).
  ...
  -
    ...
    (*$OS_STAT_SEM*)
    assert (exists tid p, TcbMod.get v'13 tid = Some (p, wait (b, i7))). 
    {
      apply list_app_in in H2.
      destruct H2.
      - 
        assert(exists tid p, TcbMod.get v'14 tid = Some (p, wait (b, i7))). 
        ...
        substs; eauto.
      -
        assert(exists tid p, TcbMod.get v'15 tid = Some (p, wait (b, i7))).  
        { eapply TCBList_P_in_list_get_some_sem; eauto. }
        mytac.
        pose proof H x.
        rewrite H8 in H10.
    ...
    mttac
      ...
      ltac: (fun H => unfolds in H; destruct H; mytac).  
    (* clear H4 H11 H12. *)
...
Lemma tick_rdy_ectr_imp_x:
  forall (v'33 : list (vallist * vallist)) (v'31 v'10 : val) 
    ...
    get_ectr (Vptr (v'37, Int.zero)) (Vptr x)
      (v'33 ++ (xx :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: nil, x0) :: nil) =
      Some
        (xx :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: nil, x0) ->
    tick_rdy_ectr
      (v'31
         :: v'10
         :: Vptr (v'37, Int.zero)
         ...
         :: Vint32 i2 :: Vint32 i1 :: Vint32 i0 :: nil)
      (Vptr x)
      (v'33 ++ (xx :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: nil, x0) :: v'34) =
      Some
        (v'33 ++
           (xx :: Vint32 (i8&ᵢInt.not i0) :: Vint32 i10 :: v :: x5 :: v'24 :: nil,
             update_nth_val (Z.to_nat (Int.unsigned i2)) x0 (Vint32 (x1&ᵢInt.not i1))) :: v'34). 
Proof.
