...
Ltac fold_AOSEvent :=
  transform_pre
    AOSEvent_fold
    ltac:(sep cancel node; sep cancel AOSEventTbl). 
...
Ltac fold_AEventNode :=
  transform_pre
    AEventNode_fold
    ltac:(sep cancel AOSEvent; sep cancel AEventData). 
...
Ltac compose_evsllseg :=
  transform_pre
    lzh_evsllseg_compose
    ltac:(sep cancel AEventNode;
          sep cancel evsllseg;
          sep cancel evsllseg). 

Lemma AECBList_fold:
  forall s P l ecbls els tcbls p,
    s |= evsllseg p Vnull l ecbls ** GV OSEventList @ (Tptr OS_EVENT) |-> p ** P ->
