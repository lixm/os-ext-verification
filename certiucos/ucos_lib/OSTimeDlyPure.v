...
Lemma R_PrioTbl_P_hold1:
  forall vl tcb tid prio st st' adrv,
    R_PrioTbl_P vl tcb adrv ->
    TcbMod.get tcb tid = Some (prio, st) ->
    R_PrioTbl_P vl (TcbMod.set tcb tid (prio, st')) adrv.
Proof.
  intros.
...
Lemma prio_available_hold_same_prio:
  forall tls pip tid p st st',
    TcbMod.get tls tid = Some (p, st) ->
    prio_available pip tls ->
    prio_available pip (TcbMod.set tls tid (p, st')).
Proof.
...
(* forall ecbls tcbls tid prio st st', *)
(* TcbMod.get tcbls tid = Some (prio, st) -> *)
(* RH_TCBList_ECBList_MUTEX_OWNER_P ecbls tcbls -> *)
(* RH_TCBList_ECBList_MUTEX_OWNER_P ecbls (TcbMod.set tcbls tid (prio, st')). *)
(* Proof. *)
(* intros. *)
(* funfold H0. *)
(* unfolds. *)
  ...
  forall a b c d e f g h i j st rtbl p t,
    R_TCB_Status_P
      (a
         ...
         :: g
         :: h :: i :: j :: nil)
      rtbl (p, t) -> (Int.eq st ($ OS_STAT_RDY) = false \/ Int.eq e ($ 0) = false) ->
    ~(t = rdy).
Proof.
  introv Hr Heq.
  unfolds in Hr.
