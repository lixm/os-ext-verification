...
Proof.
  ...
  inverts H7.
  simpl in H23.
  lets Heq :  math_prop_divu_ltmod  H6 H13 H17 H19 H23.
  subst i2.
  auto.
Qed.
  ...
  forall  (qptr qst qend qin qout : val) (size : int32) 
    ...
    RLH_ECBData_P
      (DMsgQ l
             ...
             mblk ml) A -> exists qmax pwl wl, A = (absmsgq nil qmax pwl, wl).
Proof.
  intros.
...
Proof.
  ...
  sep split in Hs.
  sep split; eauto.
  rename H2 into Hth_ptls. (* new add *)
  unfolds.
  unfolds  in H1.
  unfolds in   H0.
  splits.
...
Lemma R_ECB_ETbl_P_high_tcb_block_hold :
  forall l  vl egrp v2 v3 v4 v5 etbl tcbls ptcb prio st m m' y bity bitx ey time av petbl pegrp petbl_addr,
    Int.unsigned prio < 64 ->
    R_PrioTbl_P vl tcbls av->
    R_ECB_ETbl_P l
                 (V$OS_EVENT_TYPE_Q
                    :: Vint32 egrp
                    :: v2 :: v3 :: v4 :: v5 :: Vint32 pegrp :: petbl_addr :: nil,
                  etbl,petbl) tcbls ->
    TcbMod.get tcbls ptcb = Some (prio, st, m) ->
    y = Int.shru (prio) ($ 3) ->
    bity = ($ 1) <<ᵢ y ->
...
Proof.
 ...
 intros.
  ...
  funfold H7; unfolds; simpl;eauto.
  eapply H7 in H9.
   unfold V_OSTCBStat in H9.
  unfold  V_OSTCBEventPtr in H9.
  simpl in H9; eauto.
...
Lemma TCBList_P_tcb_block_hold :
    ...
    TCBList_P (Vptr ptcb) ((v1::v2::(Vptr qid)::m'::(Vint32 time)::(Vint32 ($ OS_STAT_Q))::(Vint32 prio)::v8::(Vint32 v9)::(Vint32 v10)::v11::nil)::vl) 
...
Proof.
  ...
  exists x x0.
  exists x1.
  exists (prio,  wait (os_stat_q qid) time,m').
  splits; eauto.
  eapply tcbjoin_set; eauto.
  unfolds in H2.
  destruct x2.
...
Lemma RH_TCBList_ECBList_P_high_get_msg_hold :
  forall ecbls tcbls pcur qid m ml max wl prio  m' pwl,
    RH_TCBList_ECBList_P ecbls tcbls pcur ->
    EcbMod.get ecbls qid = Some (absmsgq (m::ml) max pwl, wl) ->
    TcbMod.get tcbls pcur = Some (prio, rdy, m') ->
    RH_TCBList_ECBList_P (EcbMod.set ecbls qid (absmsgq ml max pwl, wl)) (TcbMod.set tcbls pcur (prio, rdy, m)) pcur. 
Proof.
  introv Hr Ht He.
  unfolds in Hr.
  ...
  inverts H.
  inverts H0.
  assert ( EcbMod.get ecbls eid = Some (absmsgq (m :: x) y pqwaitset, qwaitset) /\ In tid qwaitset).
  splits; auto.
  apply Hr1 in H.
  simpljoin1.
 ...
 (* new add about postq *)
  ...
  inverts H.
  inverts H0.
  assert ( EcbMod.get ecbls eid = Some (absmsgq (m :: x) y pqwaitset, qwaitset) /\ In tid pqwaitset).
  splits; auto.
  apply Hr1 in H.
  simpljoin1.
  assert (tid = pcur \/ tid <> pcur) by tauto.
...
Lemma RH_TCBList_ECBList_P_high_block_hold :
  forall ecbls tcbls pcur qid m ml max wl prio  time m' pwl,
    RH_TCBList_ECBList_P ecbls tcbls pcur ->
    EcbMod.get ecbls qid = Some (absmsgq ml max pwl, wl) ->
    TcbMod.get tcbls pcur = Some (prio, rdy, m) ->
    RH_TCBList_ECBList_P (EcbMod.set ecbls qid (absmsgq ml max pwl, pcur::wl)) (TcbMod.set tcbls pcur (prio, wait (os_stat_q qid) time, m')) pcur. 
Proof.
  introv Hr Ht He.
  unfolds in Hr.
  destruct Hr as (Hr1 & Hr2 & Hr3 & Hr4).
  ...
  do 3 eexists;
    rewrite TcbMod.set_sem ;
    rewrite tidspec.eq_beq_true; auto.
  assert (EcbMod.get ecbls eid = Some (absmsgq x y pqwaitset, wl) /\ In tid wl) by eauto.
  apply Hr1 in H0.
  simpljoin1.
  ...
  inverts H.
  inverts H0.
  assert ( EcbMod.get ecbls eid = Some (absmsgq x y pqwaitset, wl) /\ In tid pqwaitset).
  splits; auto.
  apply Hr1 in H.
  simpljoin1.
  assert (tid = pcur \/ tid <> pcur) by tauto.
...
Lemma event_type_n_match:
  forall s P i1 i0 i2 x2 x3 v'42 i10 pegrp petbl_addr,
    s|= AEventData
