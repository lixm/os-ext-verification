...
Proof.
  ...
  -
    ...
    unfolds in Htas.
    assert ((prio', rdy) = (prio', rdy)) by auto. 
    apply Htas in H13.
    destruct H13.
    unfolds in H13.
    destruct H13.
...
Lemma nsc_isched_step:
  forall P v'0 t ct,
    can_change_aop P ->
    P ==> GV OSTCBCur @ (Tptr OS_TCB) |-> Vptr ct ** AHprio GetHPrio t ** Atrue //\\ HCurTCB ct ** [| ct =t |] ** Atrue ->
    GetHPrio ⊢ <|| ASSUME nsc;; v'0 ||> ** P ⇒ <|| spec_done None;;v'0 ||>  ** P. 
Proof.
  intros.
  apply absinfer_seq;auto.
  apply absinfer_assume;auto.
...
Proof.
  ...
  lets Hn: Hb Hy.
  simpljoin1.
  rewrite H27 in H20; inverts H20; auto. 
Qed.
...
Lemma xxx:
  forall P s v'9 v'10,
    getisr (gettaskst s) =  isrupd v'9 0%nat false ->
    getis (gettaskst s) = 0%nat :: v'10 ->
    getie (gettaskst s) = false ->
    ( forall j : nat,
...
Proof.
  ...
  sep destruct H.
  sep split in H.
  sep_lifts_skp_in H ((OSTCBCur, _N_ 0), (OSTCBCur, _N_ 1)). 
...
Lemma tcblist_get_TCBNode_P:
  forall (l: list vallist) (tcbls : TcbMod.map) (head : val) (rtbl : vallist) (t : tidspec.A) x ,
    get tcbls t = Some x ->
...
Lemma dead_not_ready:
  forall (rtbl : vallist) (tcbls : TcbMod.map) (i x i0 y : int32) (ptbl : vallist) x0,
    prio_in_tbl ($ OS_IDLE_PRIO) rtbl ->
    array_type_vallist_match Int8u rtbl ->
    length rtbl = ∘ OS_RDY_TBL_SIZE ->
    RL_Tbl_Grp_P rtbl (Vint32 i) ->
