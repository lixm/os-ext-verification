...
Lemma rh_tcbls_mqls_p_getmsg_hold:
  forall mqls tcbls ct a v vl qmax wl pwl,
    RH_TCBList_ECBList_P mqls tcbls ct ->
    EcbMod.get mqls a =
    Some
      (absmsgq (v:: vl) qmax pwl, wl) -> 
    RH_TCBList_ECBList_P (EcbMod.set mqls a (absmsgq vl qmax pwl, wl)) tcbls ct.
Proof.
  intros.
...
rewrite H2 in H0; inverts H0.
  ...
  unfold RH_TCBList_ECBList_MUTEX_OWNER_P in *.
  destruct H1 as (_ & Hrb & Hnone & Hsome).
  splits; intros;
  rewrite EcbMod.set_sem in *;
  try destruct (tidspec.beq a eid) eqn: eq1;
  try destruct (tidspec.beq a eid2) eqn: eq1;
  inverts H1; try inverts H2; tryfalse.
  eapply Hnone; eauto.
  eapply Hsome; eauto.
  destruct (tidspec.beq a eid1) eqn: eq2;
  inverts H5.
  eapply Hsome; eauto.
Qed.
    ...
    RLH_ECBData_P
      ( 
        DMsgQ l (qptr
              ...
              mblk ml) A ->
    exists qmax pwl wl, A = (absmsgq nil qmax pwl,wl).
Proof.
  intros.
  unfolds in H0.
  destruct A.
...
Proof.
  ...
  splits.
  introv Hlt.
  lets Hed :  math_le_xyz H1 H  H22; eauto.
  ...
  mauto.
  introv Hf.
  assert ( 0 <= Int.unsigned (Int.divu (i0-ᵢ$ 4) ($ 4)) )%Z.
...
Proof.
  ...
  unfold nat_of_Z.
  apply le_change.
  eapply Z2Nat.inj_le; try lia;try apply Int.unsigned_range.
  destruct H13.
  rewrite Hlen.
  ...
  forall b qptr qst qend qin outi qsize qens qblk sti l  qblkm vl hml,
    ...
    RLH_ECBData_P (DMsgQ l
                         (qptr
                            ...
                            :: qsize :: Vint32 qens :: Vptr qblk:: nil) 
                         qblkm vl) hml -> 
    exists vl' max pwl wl, hml = (absmsgq ((nth_val' (Z.to_nat ((Int.unsigned outi - Int.unsigned sti) / 4)) vl) :: vl')  max pwl, wl).
Proof.
  introv Hint Hid  Hlen Hwl Hrl .
  funfold Hwl.
  rename H12 into Hentryltsize. rename x6 into qens. rename H13 into H11.
  unfold arrayelem_addr_right in *.
  unfold qend_right, ptr_offset_right,ptr_minus,  distance in *.
  simp join.
  fsimpl.
  ...
  assert ( Int.unsigned (Int.divu (i0-ᵢ$ 4) ($ 4)) >
       Int.unsigned (Int.divu (outi-ᵢ$ 4) ($ 4)) \/
          ...
          Int.unsigned (Int.divu (outi-ᵢ$ 4) ($ 4)) \/
          Int.unsigned (Int.divu (i0-ᵢ$ 4) ($ 4)) =
        Int.unsigned (Int.divu (outi-ᵢ$ 4) ($ 4)) ) by lia.
...
Proof.
  ...
  simp join.
  rename x7 into qens;rename H13 into HV_OSQEntries; rename H14 into Hentryltsize.
  rename H15 into H13.
  unfold V_OSQEnd in *.
  unfold V_OSQSize in *.
  ...
  inverts H6.
  inverts H1.
  inverts HV_OSQEntries.
  do 8 (* 7 *) eexists;splits;eauto.
  splits;auto.
  unfolds.
  exists 0%nat.
  inverts H2.
  ...
  simpl; auto.
  rewrite <- H.
  eapply Z2Nat.inj_le; try lia.

  rewrite <- Hentryltsize.  int auto. split. int auto. int auto.
  change (Int.unsigned ($ 1) * Int.unsigned ($ 4)) with ( Int.unsigned ($ 4)). 
...
Lemma get_wellformedosq_end':
  forall x qptr st b i qin size ens qfr qend, 
    ens = Vint32 x ->
    Int.ltu Int.zero x = true ->  (* new add *)
...
Proof.  
  ...
  intros.
  funfold H.
   rename x7 into qens; rename H12 into Hensltsize; rename H13 into H11. (* new add *)
  unfold WellformedOSQ in *.  
  do 8 (* 7 *) eexists;splits;eauto;try  unfolds; simpl; eauto.
  splits;auto.
  unfolds.
  ...
  try  unfolds; simpl; eauto. 
  int auto. 
  2:  change (Int.unsigned ($ 1) * Int.unsigned ($ 4)) with ( Int.unsigned ($ 4));
  eapply Int.unsigned_range_2. 
  split. 2:int auto. int auto.
Qed.



Lemma msgqlist_p_compose:
  forall  p qid mqls qptrl1 qptrl2 i i1 a x3 p' v'41 v'42 i0 x6 (*add v'42 i0 x6*)
   ...
   R_ECB_PETbl_P qid
    ...
    ECBList_P p Vnull (qptrl1 ++ (((V$OS_EVENT_TYPE_Q
                                    :: Vint32 i :: Vint32 i1 :: Vptr a :: x3 :: p' :: Vint32 i0 :: x6 ::  nil, v'41),v'42)::nil) ++ qptrl2) (msgqls1 ++ (msgq::nil) ++msgqls2) mqls tcbls.
Proof.
  intros.
  simpl.
...
Proof.
  ...
  funfold Hwf.
  rename x9 into qens; rename H12 into Hensltsize; rename H13 into H11.
  unfold arrayelem_addr_right in *.
  unfold qend_right, ptr_offset_right,ptr_minus,  distance in *.
  simp join.
  fsimpl.
  ...
  assert ( Int.unsigned (Int.divu (i0-ᵢ$ 4) ($ 4)) >
         ...
         Int.unsigned (Int.divu (x-ᵢ$ 4) ($ 4)) \/
         Int.unsigned (Int.divu (i0-ᵢ$ 4) ($ 4)) =
           Int.unsigned (Int.divu (x-ᵢ$ 4) ($ 4))) by lia.
  ...
  apply H18 in Ha1.
  lets Has : vallist_seg_prop_eq Ha1.
  lets Heq :  math_out_start_eq'   H11; try eapply H; eauto.
  rewrite H1 in *.
    rewrite <- Heq.
  rewrite <- Has.
  simpl in H19.
