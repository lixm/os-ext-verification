...
(*prove v'10 and v'12 are not nill, so we can unfold the mqsllseg for one node*)
  ...
  hoare_split_pure_all.
  match goal with H: V_OSEventListPtr _ = Some _ |- _ => unfolds in H; simpl in H; tryfalse end.

  hoare_split_pure_all.
  destruct H3.
  assert(Hex: exists v1 v2 v3 v4 v5 v6 tail, v = v1::v2::v3::v4::v5::v6::tail).
  { apply struct_type_vallist_match_os_event; auto. }
  simpljoin1.

  Require Import seplog_pattern_tacs. 

  hoare_lifts_skp_pre (evsllseg, _N_ 0).
  inverts H3. (* Vptr _ = Vptr _ *) 
  hoare forward.
 
(*L9: if(p1 != pevent) *)
  unfolds in H18; simpl in H18; inverts H18.
  hoare_lifts_trms_pre (evsllseg, _N_ 1).
...
(* show that the loop inv holds after the loop body *)
  destruct H3. 
  (*1: the case of x=0*)  
  left.
...
(*   sep cancel A_dom_lenv. *)
  eapply evsllseg_append1; eauto.
  {
    ...
    -
      ...
      sep_lifts_skp_in H3 (evsllseg, _N_ 1).
...
(*prove a property on the list*)
  ...
  {
    pose proof ecbls_ptr_distinct_get_last_eq
      (v'9 ++ (x :: x0 :: x1 :: x2 :: x3 :: Vptr(v'22, Int.zero) :: x5, v'14) :: nil)
      ((x6 :: x7 :: x8 :: x9 :: x10 :: v'19 :: x12, v0) :: v'10) (e::v'4) (e0::v'5) (Vptr (v'22, Int.zero)).
    rewrite <- H8 in Hx.
    apply H6; auto.
  ...
  { 
    ...
    {
      sep_lifts_skp_in H21 (evsllseg, _N_ 1).      
...
(*   sep cancel Aier. *)
  ...
  assert (s |= evsllseg v'6 v'19 (v'9++(x :: v'17 :: x1 :: x2 :: x3 :: v'19 :: x5, v'14) :: nil) (v'11 ++ v'15 :: nil) **
                       evsllseg v'19 Vnull v'5 v'3 **
                       p_local OSLInv tid (logic_val (V$ 1) :: lg')).
  sep cancel p_local.
  sep cancel 1%nat 2%nat.
  sep_lifts_trms_in H21 (evsllseg, Astruct, AOSEventTbl, AEventData).
  eapply evsllseg_append1; eauto.
