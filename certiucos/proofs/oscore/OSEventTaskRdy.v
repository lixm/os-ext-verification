...
Lemma PrioWaitInQ_RLH_ECB_ETbl_P_tcbls_ex :
  forall prio osevent etbl l tcbls te d,
    ...
    exists tid st,
      get tcbls tid = Some ($ prio, st) /\ rel_edata_stat l d st.
Proof.
  intros.
  unfold RLH_ECB_ETbl_P in H2; simp join.
  unfold RLH_ECB_ETbl_SEM_P in H2; lets Hx2: H2 H; clear H2.

  destruct d; simpl in H1; substs.
  apply Hx2 in H0; simp join; do 2 eexists; split; simpl; eauto.
Qed.
...
Lemma tcbls_get_some_TCBList_P_ptr_in_tcblist :
  forall vltcb head tcbls tid pr st rtbl,
    get tcbls tid = Some (pr, st) ->
...
Proof.
  ...
  rewrite eq1.
  rewrite H1.
  assert (TcbMod.get x1 tid = Some (pr, st)).
  {
...
Lemma tcbdllseg1_init_pre :
  forall F sd I r ri s linv tid P Q vltcb head headprev tail tailnext,
    {|F, sd, linv, I, r, ri|}|-tid
               {{tcbdllseg1 head headprev tail tailnext vltcb nil **
...
Lemma tcbdllseg1_split_pre :
  forall F sd I r ri s linv tid P Q vltcb head headprev tail tailnext p holes,
    ...
    {|F, sd, linv, I, r, ri|}|-tid 
        {{ EX vl, node p vl OS_TCB_flag **
                    tcbdllseg1 head headprev tail tailnext vltcb ((p,vl)::holes) **
                    [|tcbdllseg1_get_node p head vltcb = Some vl|] **  P}}
        s
        {{Q}} ->
...
Lemma tcbdllseg1_node_join_pre1 :
  forall F sd I r ri s linv tid P Q vltcb head headprev tail tailnext b holes vl vl',
    ...
    {|F, sd, linv, I, r, ri|}|-tid
       ...
       {|F, sd, linv, I, r, ri|}|-tid
                                    {{ Astruct (b, Int.zero) OS_TCB_flag vl' ** tcbdllseg1 head headprev tail tailnext vltcb (((Vptr (b, Int.zero)),vl)::holes) ** P}}
                                    s {{Q}}.
Proof.
  intros.
...
Proof.
  ...
  lets fff: H0 H1 H2.
  unfold nat_of_Z in *.
  assert ( (Z.to_nat (Int.unsigned y)) <> (Z.to_nat (Int.unsigned (prio1>>ᵢ$ 3)))
           \/ (Z.to_nat (Int.unsigned y)) = (Z.to_nat (Int.unsigned (prio1>>ᵢ$ 3))) ). 
  tauto.
  elim H5; intros.
...
Lemma RHL_TCB_Status_Wait_P_rel_edata_stat_rel_edata_tcbstat :
  forall x13 x12 x11 x10 i7 i6 i5 i4 i3 i2 i prio st eid d rtbl,
...
Proof.
  ...
  clear - H H0; unfolds in H.
  simpl in H0; simp join.
  pose proof H prio eid; clear H.
  elim H0; auto; intros; simp join.
...
(**)
  ...
  (*ptr ′ → OSEventGrp &= ∼ os_code_defs.bity ′*)
  hoare forward; pure_auto.
  ...
  apply disj_combine with (S:=(
   EX egrp',
     [| RL_Tbl_Grp_P (update_nth_val (Z.to_nat (Int.unsigned y)) etbl
                        (val_inj (and (Vint32 erow) (val_inj (negint (Vint32 bitx)))))) (Vint32 egrp')
        /\ Int.unsigned egrp' <= 255|] **
      <|| v'18 ||> **
      LV ptr @ os_ucos_h.OS_EVENT ∗ |-> Vptr (v'26, Int.zero) **
      Astruct (v'26, Int.zero) os_ucos_h.OS_EVENT
        (Vint32 i0 :: Vint32 egrp' :: Vint32 i1 :: x2 :: x3 :: x4 :: nil) ** 
      Aarray v'24 (Tarray Int8u ∘OS_EVENT_TBL_SIZE)
      ...
      p_local OSLInv ct (logic_val (V$ 1) :: v'23) **
  ...
  hoare_split_pure_all.
  hoare lift 15%nat pre. (* AEventData *) 
  ...
  assert(exists tid st, get tcbls tid = Some ((y<<ᵢ$ 3)+ᵢx, st) /\ rel_edata_stat (v'26, Int.zero) v'17 st) as ex_tid_prio_rel.
  {
    ...
    unfolds.
    clear - prio_lt64 y_lt8 x_lt8 H9 H10 nth_val_etbl unmap_erow erow_lt256 erow_ne0 y_shl3_shr3 y_shl3_and7.
  ...
  destruct ex_tid_prio_rel as [tid [st [tcbls_tid_prio rel_ed_st]]].
...
(*   rename v'19 into head; rename v'20 into headprev;
  ...
  destruct H23.
  assert (exists i,
             nth_val' (Z.to_nat (Int.unsigned y)) rtbl = Vint32 i /\
  ...
  assert (forall x,
             prio_in_tbl x rtbl ->
             prio_in_tbl x (update_nth_val (Z.to_nat (Int.unsigned y)) rtbl (Vint32 (Int.or rrow bitx))))
    as prio_in_tbl_preserv. 
  { 
    intros.
