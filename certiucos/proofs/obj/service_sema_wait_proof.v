...
Lemma sobjoper_absimp_idxerr:
  forall sch idx P vptr verr,
    ...
    can_change_aop P ->
    absinfer sch
      (<|| semwait ( Vint32 idx :: vptr :: verr :: nil) ||>  ** P) 
...
Lemma sobjoper_absimp_kobjerr:
  forall sch idx P objs vptr verr,
    ...
    can_change_aop P ->
    absinfer sch
      (<|| semwait ( Vint32 idx :: vptr :: verr :: nil) ||>  ** HObjs objs ** P) 
...
Lemma sobjoper_absimp_get_kobj:
  forall sch idx P objs ptr attr vptr verr,
    ...
    absinfer sch
      (<|| semwait (Vint32 idx :: vptr :: verr :: nil) ||>  ** HObjs objs ** P) 
      (<|| spec_code_cons (sem_pend (vptr :: verr :: nil)) (semwait_ret (| verr :: nil |)) ||> ** HObjs objs ** P).
Proof.
  infer_solver 2%nat.
Qed.
...
Lemma sobjoper_absimp_kobj_ret:
  forall sch P verr,
    can_change_aop P ->
    absinfer sch
      (<|| semwait_ret (| verr :: nil |) ||>  ** P) 
      (<|| END (Some verr) ||> ** P).
Proof.
  infer_solver 0%nat.
Qed.

(* the proof that the function **service_sema_P** satisfies its specification *) 
Lemma service_sema_wait_Proof:  
  forall tid vl p r, 
    Some p =
      BuildPreA' os_api service_sema_P semwaitapi vl OSLInv tid init_lg ->
    Some r =
      BuildRetA' os_api service_sema_P semwaitapi vl OSLInv tid init_lg ->
    exists t d1 d2 s,
      os_api service_sema_P = Some (t, d1, d2, s) /\
